...
<link rel="stylesheet" href="<?php echo constant('URL') ?>public/css/history.css">
        ...
        <div id="history-container" class="container">
            ...
            <div id="table-container">
                <table width="100%" cellpadding="0">
                    <thead>
                        <tr>
                        <th data-sort="title" width="35%">Producto</th>
                        <th data-sort="category">Categoría</th>
                        <th data-sort="date">Fecha de compra</th>
                        <th data-sort="amount">Precio</th>
                        <th>  </th>
                        </tr>
                    </thead>
                    <tbody id="databody">
    ...
    <script>
        ...
        async function getData(){
            data = await fetch('https://claritaps.com/expense-app/expenses/getHistoryJSON')
            .then(res =>res.json())
            .then(json => json);
            this.copydata = [...this.data];
            console.table(data);
        ...
        function renderData(data){
            ...
            data.forEach(item => { 
                ...
                databody.innerHTML += `<tr>
                        ...
                        <td>${item.date}</td>
                        <td>$${item.amount}</td>
                        <td><a href="https://claritaps.com/expense-app/expenses/delete/${item.id}">Eliminar</a></td>
                    </tr>`;
            });
        }
